...
(function($) {
	...
	$.fieldValidator = function(element, options) {
		...
		fieldValidator.init = function() {
			...
			$.each($node.context.attributes, function() {
				...
				if (attr.match(/^data-[A-z0-9]+$/g)) {
					...
					var cst = "valid" + type.charAt(0).toUpperCase()
							+ type.slice(1);

					if (typeof $[cst] == 'function') {
						var constraint = new $[cst](fieldValidator,
								fieldValidator.parseAttr(type));
			...
			$('[name="' + fieldValidator.getName() + '"]').bind(event,
					function() {
		...
		fieldValidator.addViolation = function(type, msg) {
			var $exist = $container.find('[' + settings.attrErrorName + '="'
					+ type + '"]');

			if ($exist.length == 0)
				$container.append('<li ' + settings.attrErrorName + '="' + type
						+ '">' + msg + "</li>");
		};

		/**
		 * Update a message violation, add it if not exist. Useful for
		 * constraint message with {{ value }}
		 */
		fieldValidator.upViolation = function(type, msg) {
			var $li = $container.find('[' + settings.attrErrorName + '="'
					+ type + '"]');

			if ($li.length == 0)
				fieldValidator.addViolation(type, msg);
